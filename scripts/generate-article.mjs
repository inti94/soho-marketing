...
function injectAiSummary(html, meta) {
  ...
  if (s.calcPrimary && _AISUM_VALID_CALC.has(s.calcPrimary)) {
    cta = `<a class="ais-btn primary" href="${s.calcPrimary}.html">${esc(s.calcPrimaryLabel || '🧮 계산기로 바로 계산')}</a>` +
          `<a class="ais-btn ghost" href="consultation.html">무료 진단</a>`;
  } else {
    cta = `<a class="ais-btn primary" href="consultation.html">🔎 우리 가게 1:1 무료 진단</a>`;
  }
  ...
  const box = '\n<!-- AI 핵심 요약 -->\n<section class="ai-summary" aria-label="AI 핵심 요약">\n' +
    ...
    '  <div class="ais-cta">' + cta + '</div>' + relRow + '\n</section>';

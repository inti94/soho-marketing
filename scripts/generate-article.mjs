...
4. FAQ 4~5문항 + head에 FAQPage 타입 JSON-LD 별도 추가. 본문은 h2로 큰 단락을 나누고, 필요하면 h2 아래 h3 소제목으로 더 잘게 구조화.
5. 실제 숫자 예시·사례 여러 번.
...
## 계산기·관련글 — 맥락에서만 자연스럽게 (광고처럼 보이면 실패)
신뢰도가 클릭률보다 먼저다. 아래 둘은 글 흐름에 맞을 때만 넣고, 맥락이 안 맞으면 과감히 뺀다.
...
- paras: 3~4개의 짧은 문단(각 1~3문장). 흐름 = ① 공감(이 글을 왜 검색했을지) → ② "의외로 여기서 많이 놓쳐요" 식 환기 → ③ 이유/해결 → ④ 직접 계산·확인해보고 싶게. 마크업·링크 없이 순수 텍스트만.
- calcPrimary: 이 글에 가장 맞는 계산기 슬러그 1개(alba-cost-calc, weekly-pay-calc, severance-calc, insurance-4d-calc, annual-leave-calc, vat-calc, card-fee-calc, delivery-profit-calc, delivery-ads-calc, delivery-fee-real-calc, food-cost-calc, bep-calc, store-profit-calc, rent-increase-calc, loan-interest-calc, premium-calc, closure-cost-calc, startup-cost-calc, naver-ads-calc 중). 맞는 게 없으면 빈 문자열 "".
- calcPrimaryLabel: 그 계산기로 가는 버튼 문구(예: "🧮 우리 가게 부가세 계산하기"). calcPrimary가 빈 문자열이면 이 값도 "".
- related: 위 '중복 금지' 슬러그 목록 중 이 글과 직접 관련된 기존 글 슬러그 2개(없는 슬러그 지어내지 말 것).

## 구조 — 아래 템플릿을 그대로 따라 완성된 HTML 1장을 만들 것
광고 <ins> 블록, naver-site-verification 코드, sohotip.css·Pretendard·theme.css·sohotip.js·layout.js 링크/스크립트, breadcrumb·목차·관련글·공유바 구조를 템플릿에서 그대로 복사하고 내용만 새로 채워라. canonical과 og:url은 새 글 주소(https://sohotip.co.kr/<슬러그>.html)로 바꿔라.
...
const _AB_CSS = [
  ...
  '.author-byline .ab-av svg{width:22px;height:22px}',
  '.author-byline .ab-name{font-size:14px;font-weight:700;color:#191F28}',
...
function injectAuthor(html, meta) {
  ...
  // ② 상단 작성자 바이라인 (article-header 뒤)
  if (!/class="author-byline"/.test(html) && /<\/header>/.test(html))
...
const _AISUM_VALID_CALC = new Set([
  ...
  'food-cost-calc','bep-calc','store-profit-calc','rent-increase-calc','loan-interest-calc',
  'premium-calc','closure-cost-calc','startup-cost-calc','naver-ads-calc',
]);
function injectAiSummary(html, meta) {
  ...
  const paras = s.paras.slice(0, 4).map(p => `  <p>${esc(p)}</p>`).join('\n');
  let cta;
  if (s.calcPrimary && _AISUM_VALID_CALC.has(s.calcPrimary)) {
    cta = `<a class="ais-btn primary" href="${s.calcPrimary}.html">${esc(s.calcPrimaryLabel || '🧮 계산기로 바로 계산')}</a>`;
  } else {
    cta = '';
  }
  const rel = (Array.isArray(s.related) ? s.related : [])
    .filter(r => existingSlugs.includes(r)).slice(0, 2);
  const relRow = rel.length
    ? '\n  <div class="ais-links">이어서 보면 좋아요 · ' +
      rel.map(r => {
        const t = (searchData.find(a => a.url === r + '.html') || {}).title || r;
...
function normalize(html, meta) {
  ...
  // scripts: sohotip.js 앞 layout.js (없을 때만) — 상담(consult.js) 주입 제거됨
  if (!/assets\/layout\.js/.test(html))
    html = html.replace(/(<script src="sohotip\.js[^"]*"><\/script>)/,
      `<script src="assets/layout.js?v=${ASSET_V}"></script>\n$1`);
  return normalizeEmoji(injectAiSummary(injectAuthor(html, meta), meta));
}

// 6) 파일 작성 + 사이트 통합
fs.writeFileSync(`${slug}.html`, normalize(fullHtml, meta));
